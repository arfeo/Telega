...
void DcProvider::initialize() {
    ...
    if(dcsList.count() == 0) {
        ...
        if (mDcs[1]->state() < DC::authKeyCreated) {
            qDebug() << "Creating a new DC with IP- 149.154.167.51:443";
            mDcs[1]->setHost("149.154.167.51");
            mDcs[1]->setPort(443);
...
void DcProvider::onApiError() {
    ...
    qWarning() << "Api init error when connecting session to server:" << session->errorString();
