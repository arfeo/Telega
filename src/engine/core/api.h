...
public:
    ...
    qint64 messagesSetTyping(const InputPeer &peer, bool typing);
    ...
    qint64 messagesSendEncrypted(const InputEncryptedChat &inputEncryptedChat, qint64 randomId, QByteArray data);
...
Q_SIGNALS:
    ...
    void authSentCode(qint64 msgId, bool phoneRegistered, QString phoneCodeHash);
    void authSendCallResult(qint64 msgId, bool ok);
    void authSignUpAuthorization(qint64 msgId, qint32 expires, User user);
    void authSignInAuthorization(qint64 msgId, qint32 expires);
    ...
    void messagesForwardMsgsStatedMessages(qint64 msgId, QList<Message> messages, QList<Chat> chats, QList<User> users);
...
private:
    ...
    QueryMethods messagesGetLatestMessageMethods;
    ...
    void onMessagesGetLatestMessageAnswer(Query *q, InboundPkt &inboundPkt);
