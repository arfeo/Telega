...
bool Settings::loadSettings(const QString &phoneNumber, const QString &baseConfigDirectory, const QString &publicKeyFile) {
    ...
    if (!m_pubKey) {
        ...
        qDebug() << "loaded Telegram public key from file:" << publicKeyFile;
    }

    qDebug() << "loading configuration from path:" << configPath;
    QDir configDir = QDir(configPath);
    if (!configDir.exists()) {
        if (QDir::root().mkpath(configPath)) {
            qDebug() << configPath << "config directory created";
        } else {
            qDebug() << "error creating config directory: " << configPath;
            return false;
        }
    }
    ...
    if (!configFile.exists()) {
        ...
        if (configFile.open(QIODevice::WriteOnly)) {
            ...
            qDebug() << "Impossible to open file " << configFilename << " for writting default configuration";
            return false;
        }
    }

    //read settings
    qDebug() << "loading settings file" << configFilename;
    QSettings settings(configFilename, QSettings::IniFormat);
    ...
    // log readed data
    qDebug() << "testMode:" << m_testMode;
    qDebug() << "managedDownloads:" << m_managedDownloads;
    qDebug() << "lang:" << m_langCode;
    qDebug() << "authFile:" << m_authFilename;
    qDebug() << "secretChatFile:" << m_secretChatFilename;
    qDebug() << "stateFile:" << m_stateFilename;
...
void Settings::readAuthFile() {
    ...
    for (qint32 i = 0; i < n; i++) {
        ...
        qDebug() << "DC | id:" << dc->id() << ", state:" << dc->state() <<
...
void Settings::readSecretFile() {
    ...
    qDebug() << "secret chats dh version:" << mVersion;
    qDebug() << "secret chats g:" << mG;
    qDebug() << "secret chats p:" << mP.toBase64();
