...
void Connection::setupSocket() {
    ...
    setsockopt(fd, IPPROTO_TCP, 60, &maxIdle, sizeof(maxIdle)); // TCP_KEEPIDLE

    int count = 3; // send up to 3 keepalive packets out, then disconnect if no response
    setsockopt(fd, SOL_SOCKET, 10, &count, sizeof(count)); // SOL_TCP , TCP_KEEPCNT

    int interval = 2; // send a keepalive packet out every 2 seconds (after the first idle period)
    setsockopt(fd, SOL_SOCKET, 30, &interval, sizeof(interval)); // SOL_TCP , TCP_KEEPINTVL
}
