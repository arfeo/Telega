...
void DcProvider::initialize() {
    ...
    if (!mSettings->workingDcConfigAvailabe()) {
        ...
        if (mDcs[defaultDcId]->state() < DC::authKeyCreated) {
            ...
            connect(dcAuth, SIGNAL(fatalError()), this, SLOT(fatalError()));
            connect(dcAuth, SIGNAL(dcReady()), this, SLOT(onDcReady()));
...
void DcProvider::processDcReady(DC *dc) {
    ...
    if ((!mApi) && (dc->id() == mSettings->workingDcNum())) {
        ...
        connect(session, SIGNAL(sessionReady()), this, SLOT(onApiReady()));
        connect(session, SIGNAL(error()), this, SLOT(onApiError()));
...
void DcProvider::onApiError(QAbstractSocket::SocketError errorCode) {
    ...
    qDebug() << "Api init error when connecting session to server:" << session->errorString();
...
void DcProvider::onApiReady(DC*) {
    Session *session = qobject_cast<Session *>(sender());
    qDebug() << "Api connected to server and ready";
    ...
    // get the config
    connect(mApi, SIGNAL(helpGetConfigAnswer()), this, SLOT(onConfigReceived()), Qt::UniqueConnection );
...
void DcProvider::onConfigReceived(qint64 msgId, const Config &config) {
    ...
    qDebug() << "date =" << config.date();
    qDebug() << "testMode =" << config.testMode();
    qDebug() << "thisDc =" << config.thisDc();
...
//    mPendingDcs += -1; //all the received options but the default one, yet used
    ...
    Q_FOREACH (DcOption dcOption, dcOptions) {
        qDebug() << "dcOption | id =" << dcOption.id() << ", ipAddress =" << dcOption.ipAddress() <<
...
//        if ((!dc) || ((dc->host() != dcOption.ipAddress()) || (dc->port() != dcOption.port()))) {
    ...
    qDebug() << "chatMaxSize =" << config.chatSizeMax();
}
...
void DcProvider::transferAuth() {
    ...
    for (qint32 i=0 ; i < m_dcsList.size(); i++) {
        ...
        if (dc->state() == DC::authKeyCreated &&
            ...
            connect(session, SIGNAL(sessionReady()), this, SLOT(onTransferSessionReady()));
            session->connectToServer();
...
void DcProvider::onTransferSessionReady(DC *) {
    ...
    if (--mPendingTransferSessions == 0) {
        connect(mApi, SIGNAL(authExportAuthorizationAnswer()), this, SLOT(onAuthExportedAuthorization()));
        connect(mApi, SIGNAL(authImportAuthorizationAnswer()), this, SLOT(onAuthImportedAuthorization()));
