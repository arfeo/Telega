...
void WebPageObject::setCachedPage(PageObject* cachedPage) {
    if(m_cachedPage == cachedPage) return;
    if(m_cachedPage) delete m_cachedPage;
...
PageObject*  WebPageObject::cachedPage() const {
    return m_cachedPage;
}
...
void WebPageObject::setHash(qint32 hash) {
...
qint32 WebPageObject::hash() const {
    return m_core.hash();
}
...
WebPageObject &WebPageObject::operator =(const WebPage &b) {
    ...
    m_core = b;
    m_cachedPage->setCore(b.cachedPage());
...
void WebPageObject::setClassType(quint32 classType) {
    ...
    case TypeWebPageNotModified:
        result = WebPage::typeWebPageNotModified;
        break;
    default:
        result = WebPage::typeWebPageEmpty;
        break;
...
void WebPageObject::coreCachedPageChanged() {
    if(m_core.cachedPage() == m_cachedPage->core()) return;
    m_core.setCachedPage(m_cachedPage->core());
    Q_EMIT cachedPageChanged();
    Q_EMIT coreChanged();
}
