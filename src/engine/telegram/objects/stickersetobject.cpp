...
void StickerSetObject::setArchived(bool archived) {
    if(m_core.archived() == archived) return;
    m_core.setArchived(archived);
    Q_EMIT archivedChanged();
    Q_EMIT coreChanged();
}

bool StickerSetObject::archived() const {
    return m_core.archived();
}
...
void StickerSetObject::setMasks(bool masks) {
...
bool StickerSetObject::masks() const {
    return m_core.masks();
}
