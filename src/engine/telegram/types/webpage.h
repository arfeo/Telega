...
public:
    enum WebPageClassType {
        ...
        typeWebPage = 0x5f07b4bc,
        typeWebPageNotModified = 0x85849473
    };

    WebPage(WebPageClassType classType = typeWebPageEmpty, InboundPkt *in = 0);
    WebPage(InboundPkt *in);
    WebPage(const Null&);
    WebPage(const WebPage &another);
    virtual ~WebPage();

    void setAuthor(const QString &author);
    QString author() const;

    void setCachedPage(const Page &cachedPage);
    Page cachedPage() const;
    ...
    static WebPage fromJson(const QString &json);

    bool operator ==(const WebPage &b) const;
    WebPage &operator =(const WebPage &b);
...
QDebug LIBQTELEGRAMSHARED_EXPORT operator<<(QDebug debug,  const WebPage &item);
