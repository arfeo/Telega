...
ContactsFound::ContactsFound(const ContactsFound &another) :
    TelegramTypeObject(),
    m_classType(typeContactsFound)
{
    operator=(another);
...
ContactsFound &ContactsFound::operator =(const ContactsFound &b) {
...
QMap<QString, QVariant> ContactsFound::toMap() const {
...
ContactsFound ContactsFound::fromMap(const QMap<QString, QVariant> &map) {
...
ContactsFound ContactsFound::fromJson(const QString &json) {
    return ContactsFound::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const ContactsFound &item) {
    ...
    switch(item.classType()) {
    case ContactsFound::typeContactsFound:
        debug.nospace() << "classType: typeContactsFound";
