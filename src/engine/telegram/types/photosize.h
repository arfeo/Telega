...
public:
    enum PhotoSizeClassType {
        ...
        typePhotoSizeEmptySecret23 = 0xe17e23c,
        typePhotoSizeSecret23 = 0x77bfb61b,
        typePhotoCachedSizeSecret23 = 0xe9a734fa
    };

    PhotoSize(PhotoSizeClassType classType = typePhotoSizeEmptySecret23, InboundPkt *in = 0);
    PhotoSize(InboundPkt *in);
    PhotoSize(const Null&);
    PhotoSize(const PhotoSize &another);
    virtual ~PhotoSize();
    ...
    static PhotoSize fromJson(const QString &json);

    bool operator ==(const PhotoSize &b) const;
    PhotoSize &operator =(const PhotoSize &b);
...
QDebug LIBQTELEGRAMSHARED_EXPORT operator<<(QDebug debug,  const PhotoSize &item);
