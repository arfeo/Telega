...
AuthSentCode::AuthSentCode(const AuthSentCode &another) :
    TelegramTypeObject(),
    m_flags(0),
    m_timeout(0),
    m_classType(typeAuthSentCode)
...
void AuthSentCode::setNextType(const AuthCodeType &nextType) {
    if(!nextType.isNull()) m_flags = (m_flags | (1<<1));
...
void AuthSentCode::setTimeout(qint32 timeout) {
    if(timeout) m_flags = (m_flags | (1<<2));
...
AuthSentCode &AuthSentCode::operator =(const AuthSentCode &b) {
...
QMap<QString, QVariant> AuthSentCode::toMap() const {
    ...
    case typeAuthSentCode: {
        ...
        if( !m_phoneCodeHash.isEmpty() ) result["phoneCodeHash"] = QVariant::fromValue<QString>(m_phoneCodeHash);
        if( !m_nextType.isNull() ) result["nextType"] = m_nextType.toMap();
...
AuthSentCode AuthSentCode::fromJson(const QString &json) {
    return AuthSentCode::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
QDebug operator<<(QDebug debug,  const AuthSentCode &item) {
    ...
    switch(item.classType()) {
    case AuthSentCode::typeAuthSentCode:
        debug.nospace() << "classType: typeAuthSentCode";
        debug.nospace() << ", flags: " << item.flags();
        debug.nospace() << ", type: " << item.type();
        debug.nospace() << ", phoneCodeHash: " << item.phoneCodeHash();
        debug.nospace() << ", nextType: " << item.nextType();
