...
MessagesStickerSet::MessagesStickerSet(const MessagesStickerSet &another) :
    TelegramTypeObject(),
    m_classType(typeMessagesStickerSet)
{
    operator=(another);
...
MessagesStickerSet &MessagesStickerSet::operator =(const MessagesStickerSet &b) {
...
MessagesStickerSet MessagesStickerSet::fromJson(const QString &json) {
    return MessagesStickerSet::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const MessagesStickerSet &item) {
    ...
    switch(item.classType()) {
    case MessagesStickerSet::typeMessagesStickerSet:
        debug.nospace() << "classType: typeMessagesStickerSet";
        debug.nospace() << ", set: " << item.set();
        debug.nospace() << ", packs: " << item.packs();
