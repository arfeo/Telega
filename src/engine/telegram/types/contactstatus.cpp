...
ContactStatus::ContactStatus(const ContactStatus &another) :
    TelegramTypeObject(),
    m_userId(0),
    m_classType(typeContactStatus)
{
...
ContactStatus &ContactStatus::operator =(const ContactStatus &b) {
...
ContactStatus ContactStatus::fromJson(const QString &json) {
    return ContactStatus::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const ContactStatus &item) {
    ...
    switch(item.classType()) {
    case ContactStatus::typeContactStatus:
        debug.nospace() << "classType: typeContactStatus";
