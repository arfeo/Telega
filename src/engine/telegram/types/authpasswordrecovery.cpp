...
AuthPasswordRecovery::AuthPasswordRecovery(const AuthPasswordRecovery &another) :
    TelegramTypeObject(),
    m_classType(typeAuthPasswordRecovery)
{
    operator=(another);
...
AuthPasswordRecovery &AuthPasswordRecovery::operator =(const AuthPasswordRecovery &b) {
...
QMap<QString, QVariant> AuthPasswordRecovery::toMap() const {
    ...
    case typeAuthPasswordRecovery: {
        ...
        if( !m_emailPattern.isEmpty() ) result["emailPattern"] = QVariant::fromValue<QString>(m_emailPattern);
        return result;
    }
        break;
...
AuthPasswordRecovery AuthPasswordRecovery::fromMap(const QMap<QString, QVariant> &map) {
    ...
    if(map.value("classType").toString() == "AuthPasswordRecovery::typeAuthPasswordRecovery") {
        ...
        QVariant _emailPattern_var = map.value("emailPattern");
        if( !_emailPattern_var.isNull() ) {
...
AuthPasswordRecovery AuthPasswordRecovery::fromJson(const QString &json) {
    return AuthPasswordRecovery::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const AuthPasswordRecovery &item) {
    ...
    switch(item.classType()) {
    case AuthPasswordRecovery::typeAuthPasswordRecovery:
        debug.nospace() << "classType: typeAuthPasswordRecovery";
        debug.nospace() << ", emailPattern: " << item.emailPattern();
        break;
    }
    debug.nospace() << ")";
    return debug;
