...
HelpSupport::HelpSupport(const HelpSupport &another) :
    TelegramTypeObject(),
    m_classType(typeHelpSupport)
{
    operator=(another);
...
HelpSupport &HelpSupport::operator =(const HelpSupport &b) {
...
HelpSupport HelpSupport::fromJson(const QString &json) {
    return HelpSupport::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const HelpSupport &item) {
    ...
    switch(item.classType()) {
    case HelpSupport::typeHelpSupport:
        debug.nospace() << "classType: typeHelpSupport";
