...
ChannelMessagesFilter::ChannelMessagesFilter(const ChannelMessagesFilter &another) :
    TelegramTypeObject(),
    m_flags(0),
    m_classType(typeChannelMessagesFilterEmpty)
{
...
ChannelMessagesFilter &ChannelMessagesFilter::operator =(const ChannelMessagesFilter &b) {
...
ChannelMessagesFilter ChannelMessagesFilter::fromMap(const QMap<QString, QVariant> &map) {
    ...
    if(map.value("classType").toString() == "ChannelMessagesFilter::typeChannelMessagesFilter") {
        ...
        QVariant _excludeNewMessages_var = map.value("excludeNewMessages");
        if( !_excludeNewMessages_var.isNull() ) {
...
ChannelMessagesFilter ChannelMessagesFilter::fromJson(const QString &json) {
    return ChannelMessagesFilter::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
/*QDebug operator<<(QDebug debug,  const ChannelMessagesFilter &item) {
    ...
    switch(item.classType()) {
    case ChannelMessagesFilter::typeChannelMessagesFilterEmpty:
        debug.nospace() << "classType: typeChannelMessagesFilterEmpty";
        break;
    case ChannelMessagesFilter::typeChannelMessagesFilter:
        debug.nospace() << "classType: typeChannelMessagesFilter";
        debug.nospace() << ", flags: " << item.flags();
        debug.nospace() << ", ranges: " << item.ranges();
        break;
    }
    debug.nospace() << ")";
    return debug;
