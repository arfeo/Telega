...
AccountPasswordSettings::AccountPasswordSettings(const AccountPasswordSettings &another) :
    TelegramTypeObject(),
    m_classType(typeAccountPasswordSettings)
{
    operator=(another);
...
AccountPasswordSettings &AccountPasswordSettings::operator =(const AccountPasswordSettings &b) {
...
AccountPasswordSettings AccountPasswordSettings::fromJson(const QString &json) {
    return AccountPasswordSettings::fromMap(QJsonDocument::fromJson(json.toUtf8()).toVariant().toMap());
}
...
QDebug operator<<(QDebug debug,  const AccountPasswordSettings &item) {
    ...
    Q_UNUSED(saver)
    debug.nospace() << "Telegram.AccountPasswordSettings(";
    switch(item.classType()) {
    case AccountPasswordSettings::typeAccountPasswordSettings:
        debug.nospace() << "classType: typeAccountPasswordSettings";
