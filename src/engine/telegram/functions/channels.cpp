...
bool Functions::Channels::editAdmin(OutboundPkt *out, const InputChannel &channel, const InputUser &userId, const ChannelAdminRights &adminRights) {
    out->appendInt(fncChannelsEditAdmin);
    if(!channel.push(out)) return false;
    if(!userId.push(out)) return false;
    if(!adminRights.push(out)) return false;
    return true;
}
...
bool Functions::Channels::exportMessageLink(OutboundPkt *out, const InputChannel &channel, qint32 id) {
...
ExportedMessageLink Functions::Channels::exportMessageLinkResult(InboundPkt *in) {
    ExportedMessageLink result;
    if(!result.fetch(in)) return result;
    return result;
}

bool Functions::Channels::toggleSignatures(OutboundPkt *out, const InputChannel &channel, bool enabled) {
...
UpdatesType Functions::Channels::toggleSignaturesResult(InboundPkt *in) {
    UpdatesType result;
    if(!result.fetch(in)) return result;
    return result;
}
...
bool Functions::Channels::getAdminedPublicChannels(OutboundPkt *out) {
    out->appendInt(fncChannelsGetAdminedPublicChannels);
    return true;
}

MessagesChats Functions::Channels::getAdminedPublicChannelsResult(InboundPkt *in) {
    MessagesChats result;
    if(!result.fetch(in)) return result;
    return result;
}

bool Functions::Channels::editBanned(OutboundPkt *out, const InputChannel &channel, const InputUser &userId, const ChannelBannedRights &bannedRights) {
    out->appendInt(fncChannelsEditBanned);
    if(!channel.push(out)) return false;
    if(!userId.push(out)) return false;
    if(!bannedRights.push(out)) return false;
    return true;
}

UpdatesType Functions::Channels::editBannedResult(InboundPkt *in) {
    UpdatesType result;
    if(!result.fetch(in)) return result;
    return result;
}

bool Functions::Channels::getAdminLog(OutboundPkt *out, const InputChannel &channel, const QString &q, const ChannelAdminLogEventsFilter &eventsFilter, const QList<InputUser> &admins, qint64 maxId, qint64 minId, qint32 limit) {
    out->appendInt(fncChannelsGetAdminLog);
    
    qint32 flags = 0;
    if(eventsFilter != 0) flags = (1<<0 | flags);
    if(admins.count() != 0) flags = (1<<1 | flags);
    
    out->appendInt(flags);
    if(!channel.push(out)) return false;
    out->appendQString(q);
    if(flags & 1<<0) if(!eventsFilter.push(out)) return false;
    if(flags & 1<<1) {
        out->appendInt(CoreTypes::typeVector);
        out->appendInt(admins.count());
        for (qint32 i = 0; i < admins.count(); i++) {
            if(flags & 1<<1) if(!admins[i].push(out)) return false;
        }
    }
    out->appendLong(maxId);
    out->appendLong(minId);
    out->appendInt(limit);
    return true;
}

ChannelsAdminLogResults Functions::Channels::getAdminLogResult(InboundPkt *in) {
