...
bool Functions::Upload::getWebFile(OutboundPkt *out, const InputWebFileLocation &location, qint32 offset, qint32 limit) {
    out->appendInt(fncUploadGetWebFile);
    if(!location.push(out)) return false;
    out->appendInt(offset);
    out->appendInt(limit);
    return true;
...
UploadWebFile Functions::Upload::getWebFileResult(InboundPkt *in) {
    UploadWebFile result;
    if(!result.fetch(in)) return result;
    return result;
}

bool Functions::Upload::getCdnFile(OutboundPkt *out, const QByteArray &fileToken, qint32 offset, qint32 limit) {
    out->appendInt(fncUploadGetCdnFile);
    out->appendBytes(fileToken);
    out->appendInt(offset);
    out->appendInt(limit);
    return true;
}

UploadCdnFile Functions::Upload::getCdnFileResult(InboundPkt *in) {
    UploadCdnFile result;
    if(!result.fetch(in)) return result;
    return result;
}

bool Functions::Upload::reuploadCdnFile(OutboundPkt *out, const QByteArray &fileToken, const QByteArray &requestToken) {
    out->appendInt(fncUploadReuploadCdnFile);
    out->appendBytes(fileToken);
    out->appendBytes(requestToken);
    return true;
}

bool Functions::Upload::reuploadCdnFileResult(InboundPkt *in) {
    bool result;
    result = in->fetchBool();
    return result;
}
