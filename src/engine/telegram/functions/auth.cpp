...
bool Functions::Auth::sendCode(OutboundPkt *out, bool allowFlashcall, const QString &phoneNumber, bool currentNumber, qint32 apiId, const QString &apiHash) {
...
bool Functions::Auth::dropTempAuthKeys(OutboundPkt *out, const QList<qint64> &exceptAuthKeys) {
    out->appendInt(fncAuthDropTempAuthKeys);
    out->appendInt(CoreTypes::typeVector);
    out->appendInt(exceptAuthKeys.count());
    for (qint32 i = 0; i < exceptAuthKeys.count(); i++) {
        out->appendLong(exceptAuthKeys[i]);
    }
    return true;
}

bool Functions::Auth::dropTempAuthKeysResult(InboundPkt *in) {
    bool result;
    result = in->fetchBool();
    return result;
}
