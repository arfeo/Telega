...
DecryptedMessage Decrypter::decryptEncryptedData(qint64 randomId, const QByteArray &bytes) {
    ...
    if (!((m_length & 15) == 8)) {
        qDebug() << "Received packet doesn't satisfy length rule (!(length & 0xF) == 1000)";
        return DecryptedMessage();
    }
    ...
    if (!mSecretChat) {
        qDebug() << "Received messsage to unknown chat. Dropping";
    ...
    if (mSecretChat && processMessage) {
        ...
        if (x == TL_DecryptedMessageLayer) {
            ...
            qDebug() << "Received in_seq_no=" << receivedInSeqNoParam << "(expected" << (mSecretChat->getOutSeqNoParam() - 2) << ")";
            qDebug() << "Received out_seq_no=" << receivedOutSeqNoParam << "(expected" << mSecretChat->getInSeqNoParam() << ")";
            ...
            if (deflatedOutSeqNo != mSecretChat->inSeqNo()) {
                if (deflatedOutSeqNo < mSecretChat->inSeqNo()) {
                    qDebug() << "Received out_seq_no (" << deflatedOutSeqNo
                                             << ") is lower than expected (" << mSecretChat->inSeqNo() << ")";
                    processMessage = false;
                } else if (deflatedOutSeqNo > mSecretChat->inSeqNo()) {
                    qDebug() << "Found gap since received out_seq_no (" << deflatedOutSeqNo
            ...
            if (deflatedInSeqNo > mSecretChat->outSeqNo()) {
                qDebug() << "deflated received in_seq_no is bigger than our's out seq no sent in last message (out_seq_no ="
            ...
            if (processMessage) {
                ...
                qDebug() << "Updating in_seq_no; Current values -> out_seq_no:" << mSecretChat->outSeqNo()
        ...
        if (processMessage) {
            ...
            if (decryptedMessage.classType() == DecryptedMessage::typeDecryptedMessageServiceSecret17 ||
                ...
                case DecryptedMessageAction::typeDecryptedMessageActionNotifyLayerSecret17: {
                    ...
                    qDebug() << "Received layer" << action.layer() << "from peer";
                    break;
                }
                default:
                    ;
...
bool Decrypter::checkKeyFingerprints() {
    ...
    if (mSecretChat) {
        ...
        qDebug() << "Stored keyFingerprint" << mSecretChat->keyFingerprint();
        if (keyFingerprint != mSecretChat->keyFingerprint()) {
            qDebug() << "Encrypted message with bad keyFingerprint for chat" << mSecretChat->chatId();
        } else {
            qDebug() << "keyFingerprints matches!!";
            result = true;
        }
    }
    return result;
...
QByteArray Decrypter::decryptEncryptedMessage() {
    ...
    qint32 x = prefetchInt();
    if (x < 0 || (x & 3)) {
        qDebug() << "Not valid value for internal data length" << x;
        return QByteArray();
    }
    ASSERT(x >= 0 && !(x & 3));
    ...
    qDebug() << "SHA1 checked and valid";

    qint32 plainDataLength = fetchInt();
    m_inEnd = m_inPtr + plainDataLength;
    qDebug() << "decrypted data length" << plainDataLength;
